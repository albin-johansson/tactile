...
void _check_object_layer_1(const entt::registry& registry)
{
...
void _check_tile_layer_2(const entt::registry& registry)
{
...
void _check_tile_layer_1(const entt::registry& registry)
{
...
void _check_layers(const Document& document)
{
  ...
  _check_tile_layer_1(registry);
  _check_tile_layer_2(registry);
  _check_object_layer_1(registry);
}

void _check_tilesets(const entt::registry& registry)
{
  ASSERT_EQ(2, registry.storage<comp::tileset>().size());

  const auto& tilesetEntity = registry.ctx<comp::active_tileset>().entity;
  ASSERT_NE(null_entity, tilesetEntity);
...
void _check_map_attributes(const entt::registry& registry)
{
...
TEST(RestoreDocumentFromIR, Test)
{
  parsing::map_parser parser;
  parser.parse_map("test-resources/yaml/map.yaml");
  ...
  _check_tilesets(registry);
  _check_layers(document);
}
