...
TEST_CASE("property::operator==", "[property]")
{
  SECTION("Reflexivity")
  {
    const core::property property;
  ...
  SECTION("Default constructed properties")
  {
    const core::property fst;
    const core::property snd;
    CHECK(fst == snd);
  }

  SECTION("Equal properties")
  {
    const core::property fst{354};
    const core::property snd{354};
    CHECK(fst == snd);
    CHECK(snd == fst);
  }

  SECTION("Different properties")
  {
    const core::property fst{123};
    const core::property snd{QStringLiteral(u"foo")};
    CHECK_FALSE(fst == snd);
    CHECK_FALSE(snd == fst);
  }
}
