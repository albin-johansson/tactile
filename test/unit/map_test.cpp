...
TEST_CASE("map::each_layer", "[map]")
{
  ...
  int count{};
  map.each_layer([&](layer_id, const core::shared_layer&) {
    ++count;
  });
...
TEST_CASE("map::remove_occurrences", "[map]")
{
  ...
  auto tileLayer = map.make_tile_layer();
  std::vector<core::position> positions;
  tileLayer->flood({}, 1_t, positions);

  map.add_layer(id, tileLayer);
  map.select_layer(id);
  map.remove_occurrences(1_t);

  tileLayer->for_each([](tile_id id) {
    CHECK(id == empty);
  });
}

TEST_CASE("map::add_tile_layer", "[map]")
{
