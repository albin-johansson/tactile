    ...
    , m_remove{addAction(icons::remove(), tr("Remove"))}
{
  ...
  connect(m_add, &QAction::triggered, this, &properties_context_menu::add);
  connect(m_remove,
          &QAction::triggered,
          this,
          &properties_context_menu::remove);
  connect(m_rename,
          &QAction::triggered,
          this,
          &properties_context_menu::rename);
  connect(m_moveUp,
          &QAction::triggered,
          this,
          &properties_context_menu::move_up);
  connect(m_moveDown,
          &QAction::triggered,
          this,
          &properties_context_menu::move_down);
  connect(m_duplicate,
          &QAction::triggered,
          this,
          &properties_context_menu::duplicate);
  connect(m_copy, &QAction::triggered, this, &properties_context_menu::copy);
  connect(m_paste, &QAction::triggered, this, &properties_context_menu::paste);
  // TODO change_type
}

void properties_context_menu::set_selected_item(const property_tree_item* item)
{
  m_copy->setVisible(item);
  m_copy->setEnabled(item);
  m_paste->setEnabled(!item);
  m_changeType->setEnabled(item);
