...
void TileLayer::SetTile(const MapPosition& pos, const tile_id id) noexcept
{
  if (InBounds(pos))
  {
...
auto TileLayer::TileAt(const MapPosition& pos) const -> Maybe<tile_id>
{
  if (InBounds(pos))
  {
...
auto TileLayer::InBounds(const MapPosition& pos) const noexcept -> bool
{
