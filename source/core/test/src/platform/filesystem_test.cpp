...
TEST(Filesystem, StripHomeDirectoryPrefix)
{
  ...
  const auto* abc = TACTILE_NATIVE_STR("/a/b/c");
  const auto* foobar = TACTILE_NATIVE_STR("/foobar");

  EXPECT_EQ(strip_home_directory_prefix(home_dir + abc, home_dir),
            TACTILE_NATIVE_STR("~") + NativeString {abc});
  EXPECT_EQ(strip_home_directory_prefix(home_dir + foobar, home_dir),
            TACTILE_NATIVE_STR("~") + NativeString {foobar});
