...
/// \trace tactile::Color::to_uint32_abgr
TEST(Color, ToUInt32ABGR)
{
  EXPECT_EQ(kColorBlack.to_uint32_abgr(), uint32 {0xFF000000});
  EXPECT_EQ(kColorWhite.to_uint32_abgr(), uint32 {0xFFFFFFFF});
  EXPECT_EQ(kLimeGreen.to_uint32_abgr(), uint32 {0xFF32CD32});
  EXPECT_EQ(kDarkBlue.to_uint32_abgr(), uint32 {0xFF8B0000});
  EXPECT_EQ(kHotPink.to_uint32_abgr(), uint32 {0xFFB469FF});
}
