...
{
  rows.reserve(rows.size() + sign_cast(n));
...
{
  for (auto& row : rows) {
    row.reserve(row.size() + sign_cast(n));
    for (MatrixExtent::value_type i = 0; i < n; ++i) {
...
DenseTileMatrix::DenseTileMatrix(const MatrixExtent& extent)
  : mExtent {extent}
{
  mRows.reserve(sign_cast(mExtent.rows));
  mRows.assign(sign_cast(mExtent.rows),
               Vector<TileID>(sign_cast(mExtent.cols), kEmptyTile));
}
...
auto DenseTileMatrix::at(const MatrixIndex index) -> TileID
{
  ...
  return mRows[sign_cast(index.row)][sign_cast(index.col)];
}
