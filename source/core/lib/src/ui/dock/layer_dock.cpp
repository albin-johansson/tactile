                       ...
                       EventDispatcher& dispatcher)
{
  if (const Window window {language.get(StringID::kLayerDock)}; window.is_open()) {
    ...
    _push_side_button_group(registry, map, dispatcher);
    ImGui::SameLine();
    _push_layer_group(language, registry, map);

    _push_new_layer_popup(language, dispatcher);
  }
}

void LayerDock::_push_side_button_group(const Registry& registry,
                                        const CMap& map,
                                        EventDispatcher& dispatcher)
{
...
void LayerDock::_push_layer_group(const Language& language,
                                  ...
                                  const CMap& map)
{
  ...
  if (root_layer.layers.empty()) {
    ...
    _push_layer_list_box(language, registry, root_layer);
  }
}

void LayerDock::_push_layer_list_box(const Language& language,
                                     const Registry& registry,
                                     const CGroupLayer& root_layer)
{
  if (ImGui::BeginListBox("##LayerListBox", ImVec2 {-1.0f, -1.0f})) {
    for (const auto layer_id : root_layer.layers) {
      _push_layer_item(language, registry, layer_id, true);
    }
...
void LayerDock::_push_layer_item(const Language& language,
                                 const Registry& registry,
  ...
  if (registry.has<CGroupLayer>(layer_id)) {
    if (ImGui::TreeNodeEx("##Group",
      ...
      for (const auto& nested_layer_id : group_layer.layers) {
        _push_layer_item(language, registry, nested_layer_id);
      }
    ...
    _push_object_layer_item(meta.name.c_str());
  }
  else if (registry.has<CTileLayer>(layer_id)) {
    _push_tile_layer_item(meta.name.c_str());
  }
...
void LayerDock::_push_tile_layer_item(const char* name)
{
...
void LayerDock::_push_object_layer_item(const char* name)
{
  NameBuffer name_buffer;
