 ...
 *
 * \details This function will zero the entire buffer before writing the string into it.
 *
 * \note The string can safely be longer than what the buffer can hold, the buffer will be
 ...
 *          null character ('\0').
 *
 * \details Beware of the lifetime of the pointed-to buffer! The buffer must not be
 *          destroyed before the returned string view. This function is useful to obtain a
 *          temporary string-like object to perform queries on, use the string overload if
 *          you need to store the returned string.
 *
 * \param buffer a character buffer.
 *
 * \return a temporary string view.
 */
