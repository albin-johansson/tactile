...
///
/// \details This class is the core of a typical command pattern architecture. Where all
/// user actions that should be undoable are implemented as classed inheriting from the
/// Command class. All command classes must provide `redo()`, `undo()`, and `get_name()`
...
/// numerous commands.
///
/// \note A general aim is to get command implementations as lean as possible, calling
/// into other (non-command-aware) core APIs as much as possible to simplify testing.
class Command {
 public:
