...
[[nodiscard]] auto _create_layer(Registry& registry,
                                 Map& map,
                                 ...
                                 const LayerType type) -> Entity
{
  ...
  switch (type) {
    case LayerType::TileLayer:
      return create_tile_layer(registry, layer_id, map.extent);

    case LayerType::ObjectLayer:
      return create_object_layer(registry, layer_id);
...
[[nodiscard]] auto _determine_target_root_layer(Registry& registry, const Map& map)
    -> GroupLayer*
{
  GroupLayer* target_group = nullptr;

  if (map.active_layer != kNullEntity) {
    target_group = registry.try_get<GroupLayer>(map.active_layer);
  }

  if (!target_group) {
    target_group = registry.try_get<GroupLayer>(map.root_layer);
  }
...
auto fix_tiles_in_map(Registry& registry, Map& map) -> FixTilesInMapResult
{
  ...
  visit_tile_layers(registry, map, [&](const Entity layer_entity) {
    ...
    invoke_mn(map.extent.rows, map.extent.cols, [&](const usize row, const usize col) {
      ...
      if (!is_valid_tile_identifier(registry, map, tile_id)) {
...
auto resize_map(Registry& registry, Map& map, const TileExtent new_extent) -> Result
{
  ...
  if (old_extent.cols < new_extent.cols) {
    invoke_n(col_diff, [&] { add_column_to_map(registry, map); });
  }
  else {
    invoke_n(col_diff, [&] { remove_column_from_map(registry, map); });
  }

  if (old_extent.rows < new_extent.rows) {
    invoke_n(row_diff, [&] { add_row_to_map(registry, map); });
  }
  else {
    invoke_n(row_diff, [&] { remove_row_from_map(registry, map); });
  }

  return success;
}

auto add_new_layer_to_map(Registry& registry,
                          const Entity map_entity,
                          const LayerType type) -> Entity
{
  TACTILE_ASSERT(is_map_entity(registry, map_entity));

  auto& map = registry.get<Map>(map_entity);
  auto& map_identifiers = registry.get<MapIdentifiers>(map_entity);

  const auto layer_entity = _create_layer(registry, map, map_identifiers, type);

  auto* root_layer = _determine_target_root_layer(registry, map);
                         ...
                         const Entity root_layer_entity)
{
  ...
  auto& root_layer = (root_layer_entity != kNullEntity)
                         ? registry.get<GroupLayer>(root_layer_entity)
                         : registry.get<GroupLayer>(map.root_layer);

  attach_layer_to(root_layer, layer_entity);
}

auto duplicate_layer(Registry& registry,
                     const Entity map_entity,
                     const Entity src_layer_entity) -> Entity
{
  TACTILE_ASSERT(is_map_entity(registry, map_entity));
  TACTILE_ASSERT(is_layer_entity(registry, src_layer_entity));

  const auto& map = registry.get<Map>(map_entity);
  auto& map_identifiers = registry.get<MapIdentifiers>(map_entity);

  const auto& src_layer = registry.get<Layer>(src_layer_entity);
  const auto& src_context = registry.get<Context>(src_layer_entity);

  const auto new_layer_entity = registry.create_entity();

  auto& new_context = registry.add<Context>(new_layer_entity);
  new_context = copy_context(registry, src_context);

  auto& new_layer = registry.add<Layer>(new_layer_entity);
  new_layer = src_layer;
  new_layer.id = map_identifiers.next_layer_id++;

  if (const auto* src_tile_layer = registry.try_get<TileLayer>(src_layer_entity)) {
    auto& new_tile_layer = registry.add<TileLayer>(new_layer_entity);
    new_tile_layer.tiles = src_tile_layer->tiles;
  }
  else if (const auto* src_object_layer =
               registry.try_get<ObjectLayer>(src_layer_entity)) {
    auto& new_object_layer = registry.add<ObjectLayer>(new_layer_entity);
    new_object_layer.objects.reserve(src_object_layer->objects.size());

    for (const auto src_object_entity: src_object_layer->objects) {
      new_object_layer.objects.push_back(duplicate_object(registry, src_object_entity));
    }
  }
  else if (const auto* src_group_layer = registry.try_get<GroupLayer>(src_layer_entity)) {
    auto& new_group_layer = registry.add<GroupLayer>(new_layer_entity);
    ...
    for (const auto src_child_layer_entity: src_group_layer->children) {
      duplicate_layer(registry, map_entity, src_child_layer_entity);
    }
  }

  const auto parent_layer_entity = get_parent_layer(registry, map, src_layer_entity);
  ...
  TACTILE_ASSERT(is_layer_entity(registry, new_layer_entity));
  return new_layer_entity;
}

void remove_layer_from_map(Registry& registry, Map& map, const Entity layer_entity)
{
...
auto attach_tileset_to_map(Registry& registry,
                           const Entity map_entity,
                           const Entity tileset_entity) -> Entity
{
  ...
  const auto attached_tileset_entity =
      create_attached_tileset(registry, tileset_entity, map_identifiers.next_tile_id);
...
auto select_tileset(Registry& registry, Map& map, const Entity attached_tileset_entity)
    -> Result
{
...
auto can_tile_row_be_removed(const Registry& registry) -> bool
{
  if (is_map_document_active(registry)) {
...
auto can_tile_column_be_removed(const Registry& registry) -> bool
{
  if (is_map_document_active(registry)) {
    const auto document_entity = get_active_document(registry);
