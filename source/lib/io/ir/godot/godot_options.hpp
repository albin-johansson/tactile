 ...
 *
 * \note All directory arguments will have "res://" prepended to them, i.e. they are
 *       considered to be relative to the Godot project root.
 *
 * \details There are of course differences in how Godot and Tactile handle tilemaps, so
 *          some transformations have to be made in order to export a Tactile map to be
 *          usable within Godot.
 *
 * \details There is no one way to organize a Godot project, so we let the user specify
 *          how their repository is arranged.
 */
