...
void load(const JSON& json, String& string, const char* key)
{
  ...
  if (string.empty()) {
    ...
    throw Exception {"Invalid empty translated string!"};
  }
}
...
void load_setting_strings(const JSON& json, SettingStrings& setting)
{
  ...
  load(json, setting.saturation, "saturation");
...
void load_parse_error_strings(const JSON& json, ParseErrorStrings& err)
{
  ...
  load(json, err.bad_zstd_compression_level, "bad-zstd-compression-level");
