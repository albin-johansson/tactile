...
///
/// Command stacks are laid out according to the following diagram. New commands are added to
...
/// -  \n
///
/// The "clean" index simply refers to a specific state in the command sequence. Its sole
/// purpose is to track whether any observable changes have been made since that state. This is
/// particularly useful to track changes to documents.
///
/// The current index points to the last executed command. If a command is reverted (using the
/// `undo` function), then this index is moved to the left but the command stack is otherwise
/// untouched. Similarly, if a command is repeated after being reverted, this index is
/// incremented and shifted to the right.
class CommandStack final
{
 public:
  ...
  /// This function should be called whenever a document is saved.
  void mark_as_clean();
  ...
  ///
  /// Use this function for commands that do not need to be executed when added. For example,
  /// an interactive tool command will likely not need to be executed when pushed since the
  /// effects would have already been applied when used.
  ///
  ...
  /// Constructs a command, executes it, and places it on the stack (or merges it).
  ///
  /// \tparam T A command type.
  ///
  /// \param args Arguments forwarded to a command constructor.
  ...
  ///
  /// If the supplied capacity is smaller than the current capacity, then commands are removed
