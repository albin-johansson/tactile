...
{
  if (auto document = MapDocument::make(renderer, ir_map)) {
    return std::make_unique<MapDocument>(std::move(*document));
  }
...
{
  if (const auto* map_document = dynamic_cast<const MapDocument*>(&document)) {
    return std::make_unique<MapViewImpl>(map_document);
  }
