...
#include "tactile/editor/i18n/string_id.hpp"
...
class TACTILE_EDITOR_API ICommand {
 public:
   ...
   *
   * \param other_cmd the command to try to merged into this command.
   *
   * \return true if the supplied command was merged into this command; false otherwise.
   */
  [[nodiscard]] virtual auto merge_with([[maybe_unused]] const ICommand* other_cmd)
      -> bool
  {
    return false;
  }

  [[nodiscard]] virtual auto get_string_id() const -> StringID = 0;
};
